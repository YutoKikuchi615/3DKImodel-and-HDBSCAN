...
int Size=1000;    /*domain side length*/
int effectNUM=6;  /*topological rank*/
float nyu=0.30;   /*noise paramater*/
float Q=1.0;      /*interaction paramater*/
float R=100;      /*metric range*/
float alpha = 1;  /*distance-weight parameter*/
int RS = 1;       /*random seed*/
...
int c=0;
int z=0;
int simulateLength = 10000;
int save = 100;
String[] saveData = new String[simulateLength/save];

int currentRS = RS;
int maxRS     = RS;

float currentNyu = nyu;
int nyuLoop = 1;
int frameLoop = 50;
float nyuStep = 0.10; 
...
void draw(){
   ...
   if(frameLoop==1||frameLoop%100==0)
   {
    StringBuilder sb = new StringBuilder();
     for (int i = 0; i < particle.size(); i++) {
      Particle p = particle.get(i);
      sb.append(p.position.x)
        .append(',').append(p.position.y)
        .append(',').append(p.position.z)
        .append(',').append(p.velocity.x)
        .append(',').append(p.velocity.y)
        .append(',').append(p.velocity.z)
        .append('\n');
      }
  saveData[z] = sb.toString();
  z++;
   }
   ...
   if(frameLoop==simulateLength || !(va>0)){
    ...
    if(currentRS < maxRS){
      ...
      particle.clear();
      saveData = new String[simulateLength/save+1];
      Va = 0;
      z  = 0;
      frameLoop = 0;
     ...
     if(Size > 1000.0){
   noLoop();
   println("All simulation finished");
   }
   frameLoop += 1;
}
...
float f33(PVector n,float theta){
 ...
 String makeFileName(float Qval, float nyuVal, long seedVal) {
  String qStr   = nf(Qval,   0, 2); 
  String nyuStr = nf(nyuVal, 0, 2);  
  return + Size + "vect(" + qStr + "," + nyuStr + "," + seedVal + ").txt";
   }
